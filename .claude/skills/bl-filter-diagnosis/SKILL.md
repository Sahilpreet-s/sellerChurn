...
To check: call `GET /api/v1/sellers` and filter by `category` and `churnCause`. If 2+ sellers in the same category show BL quality as churn cause, it is systemic.
...
In the outcome form:
- `churnReasons`: select `lead_quality`
- `competitorMentioned`: fill if seller mentioned one
- `commitmentByExec`: "BL filter ticket #{N} raised. Manual lead forwarding weekly until fix. ETA 2 weeks."
