...
| Prior Churn | `priorChurn: true` | Flag 2.3x re-churn risk -- handle with heightened care |
...
{}
```
The response contains guide sections (`title`, `pitch`, `actions`). If the response includes `"source": "nightly_batch"` and a `computedAt` timestamp, this is a pre-computed guide from last night's batch. Present every section in the brief.

If `source` is absent or `computedAt` is missing, the guide was generated in real-time (takes ~15 seconds) -- this means the nightly batch has not run yet.

### 5. Format the brief

```
--- SELLER BRIEF: {Name} ({ID}) ---
Risk: {Critical/High/Monitor} | Renewal: {N} days | ARR: Rs.{X}k
