...
### Unknown Competitor
If the seller mentions a competitor not listed above, read `references/competitor-intel.md` for any known intelligence. If not found:
1. Ask the seller what specifically attracted them to the competitor ("unki kaunsi cheez ne interest kiya?")
2. Listen for the real objection — competitor mention is often a proxy for an underlying platform problem
3. Address the underlying problem first, then use the general switching cost argument

## General Switching Cost Script
Use this when the specific competitor is unknown or when the seller is still in early evaluation:
