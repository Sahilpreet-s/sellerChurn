...
4. Makes a single LLM call that simultaneously extracts call insight from transcripts AND generates a personalized retention guide
5. Persists everything to `seller_computed` -- the API serves pure DB reads after this

Rate-limited to one seller every ~13 seconds to stay within LLM provider quota.
...
If the user wants to confirm enrichment worked, call the guide endpoint and check the response:
```
POST http://localhost:8080/api/v1/sellers/{sellerId}/guide
Content-Type: application/json
{}
```
A successful batch will return `"source": "nightly_batch"` and a `computedAt` timestamp in the response.
